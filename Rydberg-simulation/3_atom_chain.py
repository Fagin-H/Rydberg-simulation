...
def doallt(listvar):#,collapse=None):
    qubits = listvar[0]
    times = listvar[1]
    T = listvar[2]
    input_state, output_basis = makeinputoutput(len(qubits))
    H = makehamt(qubits,T)
    return makeplot(H,times,input_state,output_basis,qubits,None)

from multiprocessing import Pool

listvar = [[qubits,times,Temp]]*32
#print(listvar[0])

if __name__ = __main__:
    p = Pool(2)
    datas = p.map(doallt,listvar)

newdata = np.average(np.array(datas),axis=0)
#plt.plot(times,newdata)
#plt.save("~/Scratch/GroupProject/results/data.png")


np.save('/home/ucapcli/Scratch/GroupProject/results/newdata.npy',newdata)

np.save('/home/ucapcli/Scratch/GroupProject/results/data.npy',datas)
    

#doallt(qubits,times,Temp)
end = timeit.timeit()
t = end - start
print('Time' + str(t))
